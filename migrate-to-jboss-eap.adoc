...
TIP: kbd:[CTRL+p]（MacOSXではkbd:[CMD+p]）で、ファイルを素早く開くことができます。表示されるテキストボックスにファイル名を入力し始め、生成されるリストからファイルを選択するだけです。
...
[INFO]
====
このセクションでは、レポートの中から次の2つの問題を取り上げます。:

image::report_applifecycle_issues.png[Lifecycle Issues]
====

まず、Monolith アプリケーションの問題を修正します。
_StartupListener.java_ ファイルの _Hints_ 内の `WebLogic ApplicationLifecycleListenerEvent[rule-id:xxx]` をクリックします。
...
[INFO]
====
このセクションでは、移行レポートの下記の部分を修正することを考えます:
...
=== 7. Weblogic EJB Descriptorsの削除

[INFO]
====
このセクションと次のいくつかのセクションでは、報告書のこの部分を取り上げます。
...
[TIP]
====
`weblogic-ejb-jar.xml`  ファイルのタブを開いている場合は、タブを右クリックして **Reveal in Explorer**  を選択すると、プロジェクトエクスプローラですぐに見つけることができます（下図参照）。 また、 kbd:[ALT+R] (Macでは kbd:[Option+R] )でも検索できます。
...
kbd:[CTRL+p] （Macの場合は kbd:[CMD+p] ）を使って、 `monolith/src/main/java/com/redhat/coolstore/service/InventoryNotificationMDB.java` ファイルを開き、その内容を置き換えます。
...
WebLogicアプリケーションを分析するには、 `Run` アイコンをクリックします。
...
=== 12.OpenShiftプロファイルの追加

アプリの移行が完了したので、デプロイしてテストを行い、JBoss EAPとRed Hat OpenShiftがもたらす機能の一部を探り始めましょう。
kbd:[CTRL+p] （Macの場合は kbd:[CMD+p] ）を使って、 `monolith/pom.xml` ファイルを開きます。

この `<!-- TODO: Add OpenShift profile here pass:[-->]` pom.xml に以下の設定を追加します。
...
[NOTE]
====
テンプレートは、舞台裏でちょっとした仕事をしました。 上記で入力した情報を使って、新しく移行した `.war` バイナリをデプロイするために必要なKubernetes（およびOpenShift）の"infrastructure"を作成しました。 作成されたものは以下の通りです。

* Postgres データベース _Deployment_
* クールストアのモノリス _DeploymentConfig_
* これまでのデプロイメントに必要だった _Services_, _ConfigMaps_, _Secrets_ のすべて。
* JBoss EAPを実行するようにセットアップされたコンテナにパッケージされる `.war` バイナリを期待する _BuildConfig_（画像を参照)
+
...
実行中の _postgres_ データベース（紺色の丸で囲んでいます）と、まだデプロイも起動もされていない coolstore monolithが表示されています。
これは、JBoss EAPが `.war` ファイルを実行している最初のコンテナが構築されるのを待っているため、まだデプロイも起動もされていません。 これは、`.war` ファイルを実行する最初のコンテナが構築されるのを待っているからです。

==== モノリスを CLI を使ってデプロイ

Eclipse Che ワークスペースは Kubernetes クラスタ上で実行されています。これはデフォルトの制限付き _Service Account_ で実行されているため、ほとんどのリソース タイプを作成することができません。他のモジュールを完了している場合は、すでにログインしていると思いますが、もう一度ログインしてみましょう： *Login to OpenShift* をクリックして、与えられた資格情報を入力します。
...
コマンドラインに「--follow」フラグを追加すると、S2IのビルドログがCRWのターミナルに出力されるようになります。

一方、Topology ビューに戻ると、モノリスがビルドされているのがわかります。

image::monolith_build.png[building, 400]

ビルドのアイコンをクリックすると、ビルドログが表示されます（ターミナルに送られてくる内容を反映しているはずです）:
