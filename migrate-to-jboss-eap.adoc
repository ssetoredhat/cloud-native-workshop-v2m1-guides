...
特定された移行問題を含むコードにジャンプしてみましょう。MTAエクスプローラで *monolith* ソースプロジェクトを展開します。
...
_StartupListener.java_ ファイルの _Hints_ 内の `WebLogic ApplicationLifecycleListenerEvent[rule-id:xxx]` を右クリックします。コードを開く」をクリックします。
