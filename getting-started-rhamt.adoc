...
`projects/` が選択されていることを確認し、 *Select Repository Location* をクリックし、続いて *Open* をクリックしてください:

image::crw-add-workspace-2.png[crw, 900]

新しいワークスペースを読み込むために、Webブラウザーを再読み込みする必要があるかもしれません。

プロジェクトはワークスペースにインポートされ、プロジェクトエクスプローラーに表示されます（ *OPEN EDITORS* を折りたたんでください）：
...
左側の `MTA Explorer` アイコンをクリックします。すると、新しい MTA の設定が現れます。
...
移行先の _--target_ サーバで `eap7` が選択されていることを確認します。
...
WebLogicアプリケーションを分析するには、 `Run` アイコンをクリックします。 _Run_ アイコンが見つからない場合には、 _mtaConfiguration_ をクリックする必要があることに注意してください。
