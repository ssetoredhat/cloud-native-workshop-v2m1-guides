...
=== 3. モノリスアプリケーションをプロジェクトに追加する

*Server Path* を選択して、当社のモノリシック アプリケーションを分析します。

* Server Path: `/opt/apps`

image::rhamt_add_monolith_app1.png[RHAMT Add App, 900]

=== 4. Transformation Path で _Migration to JBoss EAP 7_ を選択します。
...
[Note]
====
あなたのレポートは数秒_queued_ (待機) 状態になっているかもしれません。すぐにプログレスバーが表示され、レポートが完了したら続けられます。1分以上待たされているようであれば、ブラウザのページを更新してみてください。
====
