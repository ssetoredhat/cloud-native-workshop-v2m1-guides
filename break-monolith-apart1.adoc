...
`Hibernate ORM` はデファクトスタンダードの JPA 実装であり、オブジェクトリレーショナルマッパーの全領域を提供します。複雑なマッピングを可能にしますが、単純で一般的なマッピングを簡単にするものではありません。Hibernate ORM with Panache は、Quarkusでエンティティを簡単に楽しく書けるようにすることに焦点を当てています。
...
Quarkusアプリケーションを起動すると、デフォルトで *Tests paused* と表示されます。

CodeReady は、Quarkus アプリが `5005` 番ポート（デバッグ用）と `8080` 番ポート（Webリクエスト用）を開いていることも検出します。5005 番ポートは開かず、新しいタブでポート `8080` を開くと、CodeReady で小さな Web ブラウザが開きます:
...
継続的テストを制御するために使用できる様々なホットキーがあります。 `h` を押すと、ターミナルに以下のようなコマンドの一覧が表示されます：
...
Quarkusは、新しい実験的なDev UIも提供します。これは、devモード（mvn quarkus:devでquarkusを起動した場合）で、デフォルトで `/q/dev` で利用可能です。これにより、現在ロードされているすべての拡張機能をすばやく視覚化し、ステータスを確認し、そのドキュメントに直接アクセスできます。

`quarkus-devui` のエンドポイントをクリックしてください。

image::devui-endpoint.png[devui-endpoint, 700]

新しいWebブラウザが自動的に開き、次のような画面が表示されます：
...
当社の本番の在庫マイクロサービスは、在庫データを格納するために外部データベース（PostgreSQL）を使用します。まず、PostgreSQL の新しいインスタンスをデプロイします。_Topology_ ビューを *右クリック* して、ポップアップメニューの *プロジェクトへ追加* 内の _Database_ をクリックします。
