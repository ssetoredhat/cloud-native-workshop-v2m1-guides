...
ヘルスチェックの一般的な_結果_は、宣言されたすべてのヘルスチェック手順の論理 AND として計算されます。また、Quarkus の拡張機能では、デフォルトのヘルスチェック機能も提供されています。今回、データベース拡張モジュールを使用しているので、上の `Database connections health check（データベース接続のヘルスチェック）` が表示される理由です。
